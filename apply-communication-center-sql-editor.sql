-- ============================================================================
-- CENTRE DE COMMUNICATION — SCHÉMA (Supabase SQL Editor). Additif, idempotent.
-- ============================================================================
...
create policy messages_read on public.messages for select to authenticated using (sender_id = auth.uid() or recipient_id = auth.uid());
...
create table if not exists public.communication_campaigns (
  id uuid primary key default gen_random_uuid(),
  sujet text not null, body text, channel text not null default 'in_app',
  target jsonb not null default '{}'::jsonb, template_id uuid,
  status text not null default 'draft', scheduled_at timestamptz, sent_at timestamptz,
  recipients_count int not null default 0, opens_count int not null default 0,
...
create table if not exists public.communication_templates (
  id uuid primary key default gen_random_uuid(), nom text not null, sujet text, body text,
  created_at timestamptz not null default now()
);
...
create table if not exists public.announcements (
  id uuid primary key default gen_random_uuid(), titre text not null, body text,
  level text not null default 'info', target jsonb not null default '{}'::jsonb,
  active_from timestamptz, active_until timestamptz, status text not null default 'active',
  created_at timestamptz not null default now()
);
...
create table if not exists public.communication_log (
  ...
  channel text not null default 'email', recipient_id uuid references public.profiles(id) on delete set null,
  email text, status text not null default 'sent', sent_at timestamptz not null default now(), opened_at timestamptz
);
