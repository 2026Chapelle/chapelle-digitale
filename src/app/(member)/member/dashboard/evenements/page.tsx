...
import {
  ...
  Bell, Bookmark, Filter, LayoutGrid, List, Globe, Video, Sparkles,
} from 'lucide-react'
import { EVENEMENTS, EvenementMock } from '@/lib/mock/evenements'
...
const TODAY = new Date(2026, 4, 12) // 12 mai 2026 (currentDate)
...
export default function EvenementsPage() {
  ...
  const filtered = useMemo(
    () => EVENEMENTS.filter((e) => activeType === 'Tout' || e.type === activeType),
    [activeType],
  )
  ...
  const totalInscrits = EVENEMENTS.reduce((s, e) => s + (e.est_inscrit ? 1 : 0), 0)
  const totalUpcoming = EVENEMENTS.filter((e) => !e.est_passe && new Date(e.date) >= TODAY).length
  const totalOnline = EVENEMENTS.filter((e) => e.en_ligne && !e.est_passe && new Date(e.date) >= TODAY).length
          ...
          >
            ...
            <div className="relative flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex md:flex-col items-center md:items-end gap-2 flex-shrink-0">
                <button className="btn-gold text-xs px-4 py-2 whitespace-nowrap">
                  {featured.est_inscrit ? 'Rejoindre' : "S'inscrire"}
                </button>
                <button className="btn-royal text-xs px-3 py-2 flex items-center gap-1.5">
        ...
        {myEvents.length > 0 && (
          ...
          >
            ...
            <div className="space-y-3">
              {myEvents.map((e, i) => (
                <EventRow key={e.id} e={e} i={i} formatDate={formatDate} daysUntil={daysUntil} />
              ))}
            </div>
          </motion.div>
        )}
...
function EventRow({
  ...
  return (
    <motion.div
      ...
      className="card-royal flex flex-col md:flex-row md:items-center gap-4 md:gap-5 hover:border-gold/20 transition-all group"
    >
      <div
      ...
      <div className="flex md:flex-col items-center md:items-end gap-2 md:gap-2 flex-shrink-0">
        ...
        <button
          className={e.est_inscrit ? 'btn-royal text-xs px-3 py-1.5' : 'btn-gold text-xs px-3 py-1.5'}
        >
          {e.est_inscrit ? 'Rejoindre' : "S'inscrire"}
        </button>
      </div>
    </motion.div>
  )
